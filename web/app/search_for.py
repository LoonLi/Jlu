...
def taggle_html(html,seg_list):
	soup = BeautifulSoup(html,"html.parser")
	context = soup.get_text()
	flag = 50
	for seg in seg_list:
		flag = context.find(seg)
		if flag:
			context = context.replace(seg,'<font color="red">'+seg+'</font>')
			break
	if flag<50:
		flag+=50
	result = context[(flag-50):(flag+50)]
...
def ajust_rank(pagerank,html,count):
	return pagerank*(float(count)/len(html))*float(count)

def search_for(que):
	seg_list = jieba.cut(que,cut_all=False)
	web_list = {}
	word_list = []
	first = True
	...
	for web in web_list:
		...
		row = c.fetchall()[0]
		r_dic['url'] = row[0]
		r_dic['review'] = taggle_html(row[1],word_list)
		r_dic['title'] = get_title(row[1])
		r_dic['pagerank'] = ajust_rank(float(pagerank.get(web)),row[1],web_list[web])
		final_dic.append(r_dic)

	final_dic = sorted(final_dic,key=lambda d:d['pagerank'],reverse=True)
